...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $nomor_sp2d = $_POST['nomor_sp2d'];
    $id_registrasi = $_POST['id_registrasi'];

    // Masukkan data ke dalam database
    $stmt = $koneksi->prepare("INSERT INTO sp2d (tanggal_spm, dari, tahun_anggaran, nomor_sp2d, id_registrasi) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssi", $tanggal_spm, $dari, $tahun_anggaran, $nomor_sp2d, $id_registrasi);

    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil disimpan.'); window.location.href='datasp2d.php';</script>";
    } else {
        echo "<script>alert('Terjadi kesalahan: " . $stmt->error . "'); window.location.href='sp2d.php';</script>";
    }

    $stmt->close();
}
$query = "SELECT * FROM registrasi";
$registrasiList = get_multiple_values($koneksi, $query);
        ...
        <div class="main-content app-content">
            <div class="container-fluid">
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
                    <div>
                        <h2 class="main-content-title fs-24 mb-1">UPLOAD DATA SPM(LS,GU,TU)</h2>
                                ...
                                <form method="POST" enctype="multipart/form-data">
                                    <div class="row gy-4">
                                        ...
                                        <div class="col-md-8 col-xl-8 col-xxl-8 mb-2">
                                            ...
                                            <select id="id_registrasi" class="form-control" name="id_registrasi" required style="max-height: 200px; overflow-y: auto;">
                                                ...
                                                <?php foreach ($registrasiList as $registrasi): ?>
                                                    <option value="<?php echo htmlspecialchars($registrasi['id_registrasi']); ?>">
                                                        <?php echo htmlspecialchars($registrasi['skpd']); ?> |
                                                        <?php echo htmlspecialchars($registrasi['cv']); ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control" id="input-name" name="tanggal_spm" placeholder="Masukkan Tanggal SPM" required>
                                        </div>

                                        <!-- Date Input -->
                                        <div class="col-12">
                                            <label for="input-date" class="form-label">Dari</label>
                                            <input type="text" class="form-control" id="input-name" name="dari" placeholder="Masukkan Dari" required>
                                        </div>

                                        <!-- Date Input -->
                                        <div class="col-12">
                                            <label for="input-date" class="form-label">Tahun Anggaran</label>
                                            <input type="text" class="form-control" id="input-name" name="tahun_anggaran" placeholder="Masukkan Tahun Anggaran" required>
                                        </div>

                                        <!-- Date Input -->
                                        <div class="col-12">
                                            <label for="input-date" class="form-label">Nomor SP2D</label>
                                            <input type="text" class="form-control" id="input-name" name="nomor_sp2d" placeholder="Masukkan Nomor SP2D" required>
                                        </div>
