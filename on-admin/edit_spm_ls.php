...
$id_spm_ls = $_GET['id_spm_ls'];
$query = "SELECT * FROM spm_ls WHERE id_spm_ls = ?";
$stmt = $koneksi->prepare($query);
$stmt->bind_param("i", $id_spm_ls);
$stmt->execute();
...
if ($result->num_rows > 0) {
    ...
    echo "<script>alert('Data tidak ditemukan.'); window.location.href='dataspm_ls.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_query = "UPDATE spm_ls SET id_registrasi =?, jenis_spm =?, tanggal_spm =?, no_rekening_bank =?, nama_di_rekening_bank =?, nama_bank_cabang =?, npwp =?, dasar_pembayaran =? WHERE id_spm_ls = ?";
...
<head>
    ...
    <title> BPKPAD | EDIT SPM LS </title>
