...
$id_spm_tu = $_GET['id_spm_tu'];
$query = "SELECT * FROM spm_tu WHERE id_spm_tu = ?";
$stmt = $koneksi->prepare($query);
$stmt->bind_param("i", $id_spm_tu);
$stmt->execute();
...
if ($result->num_rows > 0) {
    ...
    echo "<script>alert('Data tidak ditemukan.'); window.location.href='dataspm_tu.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_query = "UPDATE spm_tu SET id_registrasi =?, jenis_spm =?, tanggal_spm =?, no_rekening_bank =?, nama_di_rekening_bank =?, nama_bank_cabang =?, npwp =?, dasar_pembayaran =? WHERE id_spm_tu = ?";
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil diperbarui.'); window.location.href='dataspm_tu.php';</script>";
    } else {
...
<head>
    ...
    <title> BPKPAD | EDIT SPM TU </title>
